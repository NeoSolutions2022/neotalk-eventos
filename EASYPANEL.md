...
Copie as variáveis de `easypanel.env.example` para **Environment** e troque todos os valores secretos, incluindo a senha forte do primeiro administrador. Um arquivo local chamado `.env.easypanel.local`, ignorado pelo Git, pode ser usado como fonte segura para copiar os valores.
...
- serviço `app`, porta `3000`: `plataforma.neotalk.app`;
...
Após os serviços ficarem saudáveis, entre com `ADMIN_BOOTSTRAP_EMAIL` e `ADMIN_BOOTSTRAP_PASSWORD`. Abra **Qualidade** e use o botão **Sincronizar**. Essa operação exige a sessão administrativa e o token CSRF; não é mais um endpoint público.

## 5. Verificação

- Aplicação: `https://plataforma.neotalk.app`

...
export default function LiveRoom({ recording, setRecording, time, playerMode, setPlayerMode, showToast }: {
  ...
  const completeActiveBatch = (status: "done" | "error") => {
    ...
    activeBatchRef.current.status = status;
  ...
  const enqueueBatch = (text: string) => {
    ...
    pendingBatchesRef.current.push({ id: ++batchIdRef.current, text: normalized, status: "queued" });
    refreshBatchView();
    dispatchNextBatch();
  };
  ...
  return <>
    ...
    <div className="studio-grid">
      ...
      <aside className="studio-panel">
        ...
        <div className="config-block"><label>Nome da sala<input defaultValue="Evento institucional 2026" /></label><label>Avatar 3D<select value={avatar} onChange={(event) => selectAvatar(event.target.value as AvatarId)}><option value="lia">Lia · NeoTalk</option><option value="asuna">Asuna · NeoTalk</option></select></label><div className="avatar-choice"><div className="avatar-bust"><i/><i/></div><div><b>{avatar === "lia" ? "Lia" : "Asuna"}</b><small>Avatar da sala · Libras</small></div><span>{avatarReady ? "✓" : "…"}</span></div></div>
        <div className="config-block live-queue"><div className="block-title"><b>Fila de tradução</b><small>Lotes de até 12 palavras · {processedBatches} concluídos</small></div>{batches.length ? <div className="batch-list">{batches.map((batch) => <div className={`batch-item ${batch.status}`} key={batch.id}><span>{batch.status === "translating" ? "LIA" : "FILA"}</span><p>{batch.text}</p></div>)}</div> : <div className="queue-empty"><span>⌁</span><p>{recording ? "Ouvindo o primeiro trecho…" : "Os trechos falados aparecerão aqui."}</p></div>}</div>
